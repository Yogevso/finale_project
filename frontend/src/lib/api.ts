...
class ApiClient {
  ...
  async startCollaborationSession(documentId: number): Promise<{
    ...
    const { data } = await this.client.post('/collaboration/sessions/start', { document_id: documentId })
    return data
  }

  async endCollaborationSession(sessionId: string, editsCount: number = 0): Promise<void> {
    await this.client.post('/collaboration/sessions/end', { session_id: sessionId, edits_count: editsCount })
  }
  ...
  // ========== Snapshots (NOT Versions - for collaboration saves) ==========
  ...
  async createSnapshot(
    ...
    const { data } = await this.client.post(`/collaboration/documents/${documentId}/snapshots`, options || {})
    return data
  }
  ...
  async getSnapshot(documentId: number, snapshotId: number): Promise<{
    ...
    const { data } = await this.client.get(`/collaboration/documents/${documentId}/snapshots/${snapshotId}`)
    return data
  }
  ...
  async createAutoSnapshot(
    ...
    const { data } = await this.client.post(`/collaboration/documents/${documentId}/auto-snapshot`, null, {

...
class TestAttachments:
    ...
    def test_list_attachments_after_upload(
        ...
        file_content = b"Test content"
        files = {
            "file": ("uploaded.txt", io.BytesIO(file_content), "text/plain")
        }
        client.post(
            f"/api/v1/documents/{test_document.id}/attachments",
            headers=auth_headers,
    ...
    def test_delete_attachment(
        self, client: TestClient, admin_headers: dict, test_document
    ):
    ...
    def test_viewer_cannot_upload(
        ...
        files = {
            "file": ("viewer_file.txt", io.BytesIO(file_content), "text/plain")
        }
        response = client.post(
            f"/api/v1/documents/{test_document.id}/attachments",

...
import pytest
import uuid
from app.models import Document, DocumentStatus, SavedSearch
...
class TestSearch:
    ...
    def test_search_with_category_filter(self, client, auth_headers, db, test_user):
        ...
        response = client.get(
            "/api/v1/search/?q=policy&category=security",
            headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_search_pagination(self, client, auth_headers, db, test_user):
        ...
        response = client.get(
            "/api/v1/search/?q=test&page=1&page_size=2",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()

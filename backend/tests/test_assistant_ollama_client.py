...
class TestHealth:
    def test_healthy_when_model_exists(self):
        client = OllamaClient("http://localhost:11434", "llama3.1:8b")
        with patch.object(client, 'list_models', new_callable=AsyncMock, return_value=["llama3.1:8b", "mistral"]):
    ...
    def test_unhealthy_on_connection_error(self):
        client = OllamaClient("http://localhost:11434", "llama3.1:8b")
        with patch.object(client, 'list_models', new_callable=AsyncMock, side_effect=Exception("connection refused")):
            assert _run(client.is_healthy()) is False

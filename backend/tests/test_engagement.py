...
class TestBookmarks:
    ...
    def test_add_bookmark(self, client, auth_headers, db, test_user):
        ...
        # Add bookmark
        response = client.post(f"/api/v1/engagement/bookmarks/{doc.id}", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_add_bookmark_duplicate(self, client, auth_headers, db, test_user):
        ...
        response1 = client.post(f"/api/v1/engagement/bookmarks/{doc.id}", headers=auth_headers)
        assert response1.status_code in [200, 400]  # Either created or already exists

        # Add same bookmark again
        response2 = client.post(f"/api/v1/engagement/bookmarks/{doc.id}", headers=auth_headers)
    ...
    def test_remove_bookmark(self, client, auth_headers, db, test_user):
        ...
        # Add bookmark first
        client.post(f"/api/v1/engagement/bookmarks/{doc.id}", headers=auth_headers)

        # Remove bookmark
        response = client.delete(f"/api/v1/engagement/bookmarks/{doc.id}", headers=auth_headers)
        assert response.status_code in [200, 204]

    def test_bookmark_nonexistent_document(self, client, auth_headers):
        """Try to bookmark document that doesn't exist"""
        response = client.post("/api/v1/engagement/bookmarks/99999", headers=auth_headers)
...
class TestFeedback:
    ...
    def test_get_feedback_stats(self, client, auth_headers, db, test_user):
        ...
        response = client.get(f"/api/v1/engagement/feedback/{doc.id}/stats", headers=auth_headers)
    ...
    def test_feedback_nonexistent_document(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/engagement/feedback/99999", headers=auth_headers, json={"is_helpful": True}
        )
        assert response.status_code == 404
    ...
    def test_get_my_feedback(self, client, auth_headers, db, test_user):
        ...
        # Add feedback
        client.post(
            f"/api/v1/engagement/feedback/{doc.id}", headers=auth_headers, json={"is_helpful": True}
        )

        # Get my feedback for this document
        response = client.get(f"/api/v1/engagement/feedback/{doc.id}/my", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
...
class TestReadingProgress:
    ...
    def test_list_reading_progress(self, client, auth_headers, db, test_user):
        ...
        # List progress
        response = client.get("/api/v1/engagement/progress", headers=auth_headers)
        assert response.status_code == 200
        assert isinstance(response.json(), list)
    ...
    def test_get_completed_documents(self, client, auth_headers, db, test_user):
        ...
        response = client.get(
            "/api/v1/engagement/progress?completed_only=true", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_progress_nonexistent_document(self, client, auth_headers):
        ...
        response = client.put(
            "/api/v1/engagement/progress/99999", headers=auth_headers, json={"progress_percent": 50}
        )
        assert response.status_code == 404

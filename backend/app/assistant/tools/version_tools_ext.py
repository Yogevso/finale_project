...
from app.models import ActionType, Document, User, Version
...
class GetVersionDetailsTool(BaseTool):
    ...
    async def execute(
        ...
        if v.published_by:
            ...
            published_by = (
                f"\n- Published by: {pub_user.full_name if pub_user else 'Unknown'} at {v.published_at:%Y-%m-%d %H:%M}"
                if v.published_at
                else ""
            )
        scheduled = (
            f"\n- Scheduled publish: {v.scheduled_publish_at:%Y-%m-%d %H:%M}"
            if v.scheduled_publish_at
            else ""
        )
...
class GetDocumentVersionStatsTool(BaseTool):
    ...
    description = (
        "Get version statistics for a document — total versions, published count, latest version."
    )
    parameters = {
        "type": "object",
        "properties": {
    ...
    async def execute(
        ...
        published = (
            ...
            .filter(Version.document_id == doc_id, Version.is_published.is_(True))
            .scalar()
            or 0
        )
        latest = (
...
class CancelScheduledPublishTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            "version_id": {
                "type": "integer",
                "description": "Version ID to cancel scheduled publish",
            },
        },
        "required": ["version_id"],
    }
...
class ListUnpublishedVersionsTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            "document_id": {
                "type": "integer",
                "description": "Filter to a specific document (optional)",
            },
            "limit": {"type": "integer", "description": "Max results (default 20)"},
        },
        "required": [],
    ...
    async def execute(
        ...
        for v in versions:
            ...
            title = doc.title if doc else f"Doc#{v.document_id}"
            sched = (
                f" (scheduled: {v.scheduled_publish_at:%Y-%m-%d})" if v.scheduled_publish_at else ""
            )

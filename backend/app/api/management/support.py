...
@router.post(
    "/support/tickets/from-feedback/{feedback_id}",
...
def update_ticket(
    ...
    if body.status and old_status and body.status != old_status:
        asyncio.get_event_loop().create_task(
            chat_manager.broadcast_to_ticket(
                ticket_id,
                "status_update",
                {
                    "ticket_id": ticket_id,
...
@router.get(
    "/support/tickets/{ticket_id}/messages", response_model=list[SupportTicketMessageResponse]
)
...
def assign_agent(
    ...
    assignment = svc.assign_agent(
        ticket_id, current_user, body.agent_id, is_primary=body.is_primary
    )
...
@router.delete(
    "/support/tickets/{ticket_id}/assign/{agent_id}", status_code=status.HTTP_204_NO_CONTENT
)
def unassign_agent(
    ticket_id: int,
    agent_id: int,
...
def handoff_ticket(
    ...
    assignment = svc.handoff_ticket(ticket_id, current_user, body.target_agent_id, note=body.note)
    return SupportTicketAssignmentResponse(
        id=assignment.id,
        ticket_id=assignment.ticket_id,
        agent_id=assignment.agent_id,

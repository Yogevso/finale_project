...
_REGISTRATION_CONFLICT_DETAIL = "A user with that email or username already exists"
...
class AuthService(SessionService):
    ...
    def _lock_user_row(self, user_id: int) -> User | None:
        """Re-fetch User with a row-level write-lock where supported.

        PostgreSQL uses SELECT … FOR UPDATE to serialise concurrent access.
        SQLite serialises writes at the database level, so a plain SELECT suffices.
        """
    ...
    def _create_password_reset_token(self, user: User) -> str:
        """Persist a one-time password reset token for the provided user."""
    ...
    def reset_password(self, token: str, new_password: str) -> None:
        ...
        # H-09: Revoke all active sessions — a reset usually means compromise.
    ...
    def refresh_access_token(self, refresh_token: str) -> TokenResponse:
        ...
        # M-10 / M-03: Rotate refresh token atomically — invalidate old and
        # issue new in a single commit so a crash between the two steps cannot
        # leave the user without a valid token.
    ...
    def logout(self, user_id: int, *, session_id: int | None = None) -> None:
        """Revoke the current session when available, otherwise fall back to all sessions."""

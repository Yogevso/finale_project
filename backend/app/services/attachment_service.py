...
class AttachmentService:
    ...
    async def upload_attachment(
        ...
        # Only admin/editor/super_admin can upload
        if current_user.role not in [UserRole.ADMIN, UserRole.EDITOR, UserRole.SUPER_ADMIN]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only admins and editors can upload attachments"
            )
        ...
        try:
            ...
            storage_key = storage.upload(file_stream, f"doc_{document_id}/{unique_filename}", content_type)

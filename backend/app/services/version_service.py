...
class VersionService:
    ...
    def create_version(
        ...
        # Only super_admin, admin or editor can create versions
        if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.EDITOR]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only admins and editors can create versions"
            )

        # Get next version number
        last_version = db.query(Version).filter(
            Version.document_id == document_id
    ...
    def update_version(
        ...
        # Only super_admin, admin/editor can update
        if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.EDITOR]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only admins and editors can update versions"
            )
    ...
    def publish_version(
        ...
        # Only super_admin or admin can publish
        if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only admins can publish versions"
            )
    ...
    def delete_version(
        ...
        if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only admins can delete versions"
            )

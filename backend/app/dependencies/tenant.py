...
async def get_tenant_context(
    ...
    - SYSTEM_ADMIN users have is_system_admin=True and can access all tenants
...
def require_system_admin(tenant_ctx: TenantContext = Depends(get_tenant_context)) -> TenantContext:
...
def require_tenant(tenant_ctx: TenantContext = Depends(get_tenant_context)) -> TenantContext:
    """Dependency that requires a valid tenant_id (rejects system admins without tenant scope)"""
    if tenant_ctx.tenant_id is None and not tenant_ctx.is_system_admin:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="User is not assigned to a tenant"
        )
    return tenant_ctx
